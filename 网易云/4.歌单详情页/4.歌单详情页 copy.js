window.onload = function () {
    Ajax({
        url: 'https://autumnfish.cn/login/cellphone?phone=13413422538&password=zx892110',
        success: function () {
            console.log("登录成功");
            Ajax({
                url: 'https://autumnfish.cn/login/status',
                success: function () {
                    console.log("已登录");
                }
            })
    ...
    if (type == '歌单') {
        let musicurl = defaultUrlHeader + '/playlist/detail?id=' + musicid;
        Ajax({
            url: musicurl,
            success: function (results) {
                ...
                for (let j = 0; j < 10; j++) {
                    ...
                    span_1_1.innerHTML = j + 1;
                    span_2.innerHTML = music_info.playlist.tracks[j].name;
                    span_4.innerHTML = music_info.playlist.tracks[j].ar[0].name;
                    span_5.innerHTML = music_info.playlist.tracks[j].al.name;
                }
            }
        });
        ...
        Ajax({
            ...
            success: function (results) {
                for (let j = 0; j < results.comments.length; j++) {
                    ...
                    img.src = results.comments[j].user.avatarUrl;
                    span_1.innerHTML = results.comments[j].user.nickname + '：';
                    span_2.innerHTML = results.comments[j].content;
                    let time = results.comments[j].time;
                    time = formatDateT(time);
                    time = time.split('T');
                    div_2_2_1.innerHTML = time[0];
                    span_3.innerHTML = results.comments[j].likedCount;
                    // span_2.innerHTML=music_info.playlist.tracks[j].name;
                    // span_4.innerHTML=music_info.playlist.tracks[j].ar[0].name;
                    // span_5.innerHTML=music_info.playlist.tracks[j].al.name;
                }
        ...
        Ajax({
            ...
            success: function (results) {
                for (let j = 0; j < 5; j++) {
                    ...
                    img.src = results.playlists[j].coverImgUrl;
                    div_2_1.innerHTML = results.playlists[j].name;
                    span.innerHTML = 'by ' + results.playlists[j].creator.nickname;
        ...
        my_comment_btn[0].onclick = function () {
            ...
            let sendcommenturl = defaultUrlHeader + '/comment?t=1&type=2&id=' + musicid + '&content=' + text;
            Ajax({
                url: sendcommenturl
            });
        }
